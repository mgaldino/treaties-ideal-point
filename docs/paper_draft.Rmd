...
Four of six domains show moderate-to-strong UNGA alignment ($|r| > 0.3$). V-Dem correlates more strongly than UNGA in three domains (environment, security, arms control), suggesting that dim-1 partially captures domestic liberalism alongside international alignment. Intellectual property is the exception: UNGA outperforms V-Dem ($r = 0.52$ vs. $0.46$), consistent with IP treaty participation tracking geopolitical alignment more than regime type. Investment shows near-zero correlation with both, reflecting the economic rather than political logic of BIT networks. Human rights shows a negative correlation ($r = -0.16$ with UNGA, $r = -0.29$ with V-Dem), which raises an identification concern: if the first dimension genuinely captures ILO support, we would expect at least a weakly positive correlation with these external measures. The negative sign may reflect the fact that the HR domain's anchor-defined dimension captures institutional selectivity (optional protocol ratification) rather than the broader pro-ILO orientation that UNGA votes and V-Dem scores track. Combined with the high anchor sensitivity documented in Section R2 ($r$ as low as $-0.87$ with alternative anchors), the human rights domain presents the weakest case for interpreting dim-1 as ILO support, and results for this domain should be treated with particular caution.
