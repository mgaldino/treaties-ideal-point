# Task: Rcpp Optimization of dynIRT_KD — Gemini Implementation
...
Create Rcpp (.cpp) files in `scripts/R/rcpp_gemini/`. Priority order:
...
Create `scripts/R/rcpp_gemini/dynIRT_KD_rcpp.R` that sources the Rcpp versions and replaces the R functions.
...
Save benchmark results to `outputs/rcpp_gemini_benchmark.rds` and print a summary.

## Output Files

All files in `scripts/R/rcpp_gemini/`:
- `da_step_rcpp.cpp`
...
Results in `outputs/`:
- `rcpp_gemini_benchmark.rds`

Log in `logs/`:
- `rcpp_gemini_benchmark.log`

## Constraints

- Do **NOT** modify the original R files (`scripts/R/da_step.R`, etc.)
- Create all new files in `scripts/R/rcpp_gemini/`
