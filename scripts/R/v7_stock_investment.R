...
# Prefer the merged Rcpp implementation; fall back to pure R if compilation fails.
ok_rcpp <- TRUE
tryCatch(
...
anchor_positions <- rbind(
  ...
  c( 0, -2)   # CHN: dim2 anchor (confounder axis)
)
...
saveRDS(out_A, sprintf("outputs/v7_stock_country_anchors/%s_results.rds", domain))
cat(sprintf("Saved: outputs/v7_stock_country_anchors/%s_results.rds\n", domain))
sink()
