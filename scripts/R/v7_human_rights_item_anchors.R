...
# Source components (Rcpp with pure R fallback)
rcpp_ok <- tryCatch({
...
if (!exists("compute_loglik", mode = "function") ||
    !exists("param_change", mode = "function") ||
    !exists(".kalman_one_country", mode = "function")) {
...
# Select 3 anchor items from estimated betas (Part A):
...
product_excl <- product
product_excl[c(item_A, item_B)] <- -Inf
item_C <- which.max(product_excl)

anchor_items <- c(item_A, item_B, item_C)
if (length(unique(anchor_items)) < 3L || any(!is.finite(product_excl))) {
  # Fallback: top 3 by total discrimination
...
cat("Fixed alpha:\n"); print(round(fixed_alpha, 4))
cat("Fixed betas:\n"); print(round(fixed_betas, 4))
...
# Warm-start alpha/beta from Part A; x from PCA
alpha <- alpha_A
beta <- beta_A
x_smooth <- x_start
Omega <- 0.1 * diag(K)
...
cat("Fixed alpha:\n"); print(fixed_alpha)
cat("Fixed betas:\n"); print(fixed_betas)
cat(sprintf("Rcpp ok: %s\n\n", if (rcpp_ok) "TRUE" else "FALSE"))
...
for (iter in seq_len(maxit)) {
  ...
  # Enforce fixed item anchors (alpha + beta) from Part A
  ...
  need_ll <- (iter == 1L) || (iter %% checkfreq == 0L) || (delta < thresh) || (iter == maxit)
  if (need_ll) {
