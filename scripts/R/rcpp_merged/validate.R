...
# Validation: R vs merged Rcpp equivalence

set.seed(2026)

# ---- Helper: small simulated dataset ----
...
if (!is.finite(max_diff) || max_diff > 1e-10) {
  stop(sprintf("R vs merged Rcpp mismatch: max diff %.3e exceeds tolerance", max_diff))
}

cat("Validation passed (max diff < 1e-10).\n")
